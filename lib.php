...
/**
 * Detect a stranded recorded version for enrol_prereq2.
 *
 ...
 */
function enrol_prereq2_version_is_stranded() {
    global $CFG;
    ...
    $stored = get_config('enrol_prereq2', 'version');
    ...
    $plugin = new stdClass();
    $versionfile = $CFG->dirroot . '/enrol/prereq2/version.php';
 ...
 */
function enrol_prereq2_repair_stranded_version() {
    $state = enrol_prereq2_version_is_stranded();
    if ($state === false) {
        return ['ok' => false, 'from' => '', 'to' => 0,
                'message' => get_string('versionrepair_notneeded', 'enrol_prereq2')];
    }
    ...
    add_to_config_log('version', $state['stored'], (string) $state['target'], 'enrol_prereq2');
    set_config('version', $state['target'], 'enrol_prereq2');
    ...
    return ['ok' => true, 'from' => $state['stored'], 'to' => $state['target'],
            'message' => get_string('versionrepair_done', 'enrol_prereq2',
 ...
 */
function enrol_prereq2_version_banner() {
    if (!is_siteadmin()) {
    ...
    $url = new moodle_url('/enrol/prereq2/version_repair.php', ['sesskey' => sesskey()]);
    ...
    $html .= '<strong style="font-size:15px;">Prerequisite 2 Enrolment cannot be updated on this site</strong><br>';
